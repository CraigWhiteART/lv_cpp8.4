 ...
 */
class LV_EMPTY_BASES Canvas : public ObjectView,
...
#else
    Canvas& buffer(void* buf, int32_t w, int32_t h, lv_img_cf_t cf) noexcept {
        lv_canvas_set_buffer(m_obj, buf, w, h, cf);
        return *this;
    }
#endif
...
#else
        (void)opa; // opa not supported in LVGL 8.x set_px
        lv_canvas_set_px(m_obj, x, y, color);
#endif
        return *this;
    }
...
#else
    [[nodiscard]] lv_color_t get_px(int32_t x, int32_t y) const noexcept {
        return lv_canvas_get_px(m_obj, x, y);
    }
#endif
...
#else
    Canvas& set_palette(uint8_t index, lv_color_t color) noexcept {
        lv_canvas_set_palette(m_obj, index, color);
        return *this;
    }
#endif
...
#if LV_VERSION_AT_LEAST(9, 0, 0)
    /// Initialize draw layer for drawing operations (LVGL 9.x only)
    void init_layer(lv_layer_t* layer) noexcept {
        lv_canvas_init_layer(m_obj, layer);
    }

    /// Initialize draw layer (Layer wrapper overload, LVGL 9.x only)
    template<typename L>
    void init_layer(L& layer) noexcept {
        lv_canvas_init_layer(m_obj, layer.get());
    }

    /// Finish layer and render to canvas (LVGL 9.x only)
    void finish_layer(lv_layer_t* layer) noexcept {
        lv_canvas_finish_layer(m_obj, layer);
    }

    /// Finish layer (Layer wrapper overload, LVGL 9.x only)
    template<typename L>
    void finish_layer(L& layer) noexcept {
        lv_canvas_finish_layer(m_obj, layer.get());
    }

    /// Get image descriptor (LVGL 9.x style)
    [[nodiscard]] lv_image_dsc_t* get_image() const noexcept {
        return lv_canvas_get_image(m_obj);
    }

    /// Alias for get_image() - provides LVGL 8.x style API
    [[nodiscard]] lv_image_dsc_t* get_img() const noexcept {
        return get_image();
    }

    /// Get raw buffer pointer (LVGL 9.x only)
    [[nodiscard]] const void* get_buf() const noexcept {
        return lv_canvas_get_buf(m_obj);
    }
#else
    /// Get image descriptor (LVGL 8.x)
    [[nodiscard]] lv_img_dsc_t* get_img() const noexcept {
        return lv_canvas_get_img(m_obj);
    }

    /// Alias for get_img() - provides consistent API across LVGL versions
    [[nodiscard]] lv_img_dsc_t* get_image() const noexcept {
        return get_img();
    }
#endif
...
#if LV_VERSION_AT_LEAST(9, 0, 0)
    /// Copy from another draw buffer (LVGL 9.x)
...
#else
    /// Copy buffer to canvas at position (LVGL 8.x)
    Canvas& copy_buf(const void* to_copy, lv_coord_t x, lv_coord_t y, lv_coord_t w, lv_coord_t h) noexcept {
        lv_canvas_copy_buf(m_obj, to_copy, x, y, w, h);
        return *this;
    }
#endif
